...
for i, batch in enumerate(test_dataloader): #如果是非有效的，则在进行dataset处理的时候就已经消除掉了,所以有效个数就是i值
    ...
    pr[0] = pr[0]+rouge.compute_rouge([sentence[max_idx[0]]],[sentence[max_idx[1]]])['rouge-1']['r'][0]
    pr[1] = pr[1]+rouge.compute_rouge([sentence[max_idx[1]]],[sentence[max_idx[2]]])['rouge-1']['r'][0]

    pr[2] = pr[2]+rouge.compute_rouge([sentence[max_idx[0]]],[sentence[max_idx[1]]])['rouge-2']['r'][0]
    pr[3] = pr[3]+rouge.compute_rouge([sentence[max_idx[1]]],[sentence[max_idx[2]]])['rouge-2']['r'][0]

    # pr[4] = pr[4]+rouge_sim.get_scores(sentence[max_idx[0]],sentence[max_idx[1]])[0]['rouge-1']['r']
    # pr[5] = pr[5]+rouge_sim.get_scores(sentence[max_idx[1]],sentence[max_idx[2]])[0]['rouge-1']['r']

    # pr[6] = pr[6]+rouge_sim.get_scores(sentence[max_idx[0]],sentence[max_idx[1]])[0]['rouge-2']['r']
    # pr[7] = pr[7]+rouge_sim.get_scores(sentence[max_idx[1]],sentence[max_idx[2]])[0]['rouge-2']['r']

    # pr[8] = pr[8]+rouge_sim.get_scores(sentence[max_idx[0]],sentence[max_idx[1]])[0]['rouge-l']['r']
    # pr[9] = pr[9]+rouge_sim.get_scores(sentence[max_idx[1]],sentence[max_idx[2]])[0]['rouge-l']['r']
    ...
    if i%200==0:
        print(i)
position_idx_re = [j/float(i) for j in pr]
print(position_idx_re)