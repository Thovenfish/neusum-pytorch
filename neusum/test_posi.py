...
position_idx = [0 for i in range(30)] #生成一个30成的0的值
...
for i, batch in enumerate(test_dataloader): #如果是非有效的，则在进行dataset处理的时候就已经消除掉了,所以有效个数就是i值
    ...
    for j in range(max_step):
        ...
        m_id = int(m_ids[j])
        if m_id <30:
            position_idx[m_id] = position_idx[m_id] + 1
    
    if i%200==0:
        print(i)

sum_pos = float(sum(position_idx))
position_idx_re = [j/sum_pos for j in position_idx]
